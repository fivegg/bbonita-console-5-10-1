...
public class FormMessageWidget extends Composite {
     ...
     */
    public FormMessageWidget(final FormWidget formWidget) {
     ...
     * @param fieldValue the widget field value object
     */
    public FormMessageWidget(final FormWidget formWidget, final FormFieldValue fieldValue) {

        this.allowHTMLInField = formWidget.allowHTMLInField();
     ...
     * 
     * @param formMessageData the widget data object
     * @param fieldValue the widget field value object
     */
    protected void createWidget(final FormWidget formMessageData, final FormFieldValue fieldValue) {
    ...
    protected void setHTMLContent(final FormFieldValue fieldValue) {
        String content = getStringValue(fieldValue);
        message.setHTML(content);
        //Fix Javascript execution on IE7 and IE8
        if (domUtils.isIE7() || domUtils.isIE8()) {
            String scriptPattern = "<script.*javascript.*>(.*)</script>";
            RegExp regExp = RegExp.compile(scriptPattern, "gi");
     ...
     */
    protected String getDateAsText(final FormWidget widgetData, final FormFieldValue fieldValue) {
