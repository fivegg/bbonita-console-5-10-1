 ...
 * If the default implementation of FormServiceProvider is not used, the deployment descriptor (web.xml) of the webapps (both user XP and process applications) needs to be modified in order to remove the filter BPMURLFilter. This filter is used to add the form ID in the URL if it is not present (in order to support the old URL format).<br/>
 * 
...
public interface FormServiceProvider {
     ...
     * If you use a dedicated web archive (.war) for each forms application, the forms.xml file will be in the classpath (in WEB-INF/classes), so you can read it from there.
     * 
     ...
     * @throws FormNotFoundException if no form definition file was found
     ...
     * @param migrationProductVersion The migration product version
     * @param context Map of context (containing the URL parameters and other data)
     * @param isFormPermissions indicates if the permissions are permissions to display a form or not (application permissions)
     * @return true if allowed, otherwise return false
     * @throws ForbiddenFormAccessException if the user doesn't have the rights to see the form
     * @throws SuspendedFormException if the access to the form is suspended
     * @throws CanceledFormException if the form doesn't have to be filled in anymore
     * @throws FormNotFoundException if the form cannot be found
     * @throws FormAlreadySubmittedException if the form has already been submitted and cannot be submitted twice
     * @throws ForbiddenApplicationAccessException if the user isn't allowed tu access the forms application
     ...
     */
    boolean isAllowed(String formId, String permissions, String productVersion, String migrationProductVersion, Map<String, Object> context, boolean isFormPermissions) throws ForbiddenFormAccessException, SuspendedFormException, CanceledFormException, FormNotFoundException, FormAlreadySubmittedException, ForbiddenApplicationAccessException, FormInErrorException, MigrationProductVersionNotIdenticalException, NoCredentialsInSessionException, SkippedFormException;
     ...
     * @throws VariableNotFoundException if a data to set cannot be found
     * @throws FileTooBigException if a file is too big to be uploaded
     * @throws FormNotFoundException if the form cannot be found
     ...
     * @throws FormNotFoundException if the form cannot be found
     */
    FormURLComponents getNextFormURLParameters(final String formID, Map<String, Object> context) throws FormNotFoundException;
     ...
     * @throws FormNotFoundException if the form cannot be found
     */
    Map<String, String> getAttributesToInsert(Map<String, Object> context) throws FormNotFoundException;
     ...
     * @throws FormValidationException if an error occurs during the field validation
     * @throws FormNotFoundException if the form cannot be found
     */
    List<FormValidator> validateField(List<FormValidator> validators, String fieldId, FormFieldValue fieldValue, String submitButtonId, Map<String, Object> context) throws FormValidationException, FormNotFoundException;
     ...
     */
    @Deprecated
    List<FormValidator> validateField(List<FormValidator> validators, String fieldId, FormFieldValue fieldValue, Map<String, Object> context) throws FormValidationException, FormNotFoundException;
    
    /**
     * Validate a form page with several fields
     *  
     * @param validators List of validators to use
     * @param fields Map containing the fields ids and values
     * @param submitButtonId The submit button ID
     * @param context Map of context (containing the URL parameters and other data)
     * @return a list of the validators for which the field value does not comply with the validation
     * @throws FormValidationException if an error occurs during the page validation
     * @throws FormNotFoundException if the form cannot be found
     */
    List<FormValidator> validatePage(List<FormValidator> validators, Map<String, FormFieldValue> fields, String submitButtonId, Map<String, Object> context) throws FormValidationException, FormNotFoundException;
     ...
     */
    @Deprecated
    List<FormValidator> validatePage(List<FormValidator> validators, Map<String, FormFieldValue> fields, Map<String, Object> context) throws FormValidationException, FormNotFoundException;
     ...
     * The implementation of this method can return null but the cache directory in BONITA_HOME will have to be cleared manually and the server restarted in case a new version of the application is deployed
     * @param context Map of context (containing the URL parameters and other data)
     * @return the deployment {@link Date} 
     * @throws FormNotFoundException if the form cannot be found
     * @throws IOException 
     */
    Date getDeployementDate(Map<String, Object> context) throws FormNotFoundException, IOException;
    
    /**
     * Get an application form definition accessor object (reader to access a form definition for a specific form ID).<br/>
     * In case the form definition is only based on a forms.xml file (valid with the forms.xsd) this method can just return: FormDefAccessorFactory.getXMLApplicationFormDefAccessor(formId, formDefinitionDocument, applicationDeploymentDate) 
     * 
     * @param formId the form Id
     * @param formDefinitionDocument the document
     * @param context Map of context (containing the URL parameters and other data)
     ...
     * In case the form definition is only based on an xml file (valid with the forms.xsd) this method can just return: new XMLApplicationConfigDefAccessorImpl(formDefinitionDocument)
     * 
     * @param formDefinitionDocument the document
     * @param context Map of context (containing the URL parameters and other data)
     ...
     * If want to use use attachments in your process, you need to implement this method.<br/>
     * Currently, you also need to re-implement the AttachmentDownloadServlet and the ImageDownloadServlet and configure them in the web.xml of the webapps.<br/>
     * TODO: refactor those servlets so that they use the FormServiceProvider and we don't need to re-implement them to use something else than the engine as back-end.
     * 
     * @param context Map of context (containing the URL parameters and other data)
     ...
     * @throws FormNotFoundException if the form cannot be found
     */
    String getAttachmentFileName(String attachmentName, Map<String, Object> context) throws FormNotFoundException;
    
   /**
    * Check if a specific form is in edit mode (entry form) or not (view form).<br/>
    * This method is used mostly to add an attribute in the context Map passed to the the expression resolution and connector execution methods (because you might need this information at this point).<br/>
    * This method is required because if you don't use an XML definition for a form (like we do for the automatic form generation), you need to access your back-end to know if the the form displayed is in edit or view mode.
    * 
    ...
    * @throws FormNotFoundException if the form cannot be found
    */
    boolean isEditMode(final String formID, final Map<String, Object> context) throws FormNotFoundException;
     ...
     * This method is used mostly to add an attribute in the context map passed to the the expression resolution and connector execution methods (because you might need this information at this point).<br/>
     * This method is required because if you don't use an XML definition for a form (like we do for the automatic form generation), you need to access your back-end to know if the the form displayed needs to use current values or previous values. 
     * 
     * @param context Map of context (containing the URL parameters and other data)
     * @return true if the form contains user input data, otherwise return false
     * @throws FormNotFoundException if the form cannot be found
     */
    boolean isCurrentValue(final Map<String, Object> context) throws FormNotFoundException;
     ...
     * This method is used in a very specific use case. If you don't need this feature, you can just return an empty map in your implementation of {@link FormServiceProvider}.
     * 
     * @param formID The form id to be skipped
     * @param context Map of context (containing the URL parameters and other data)
     * @return a {@link Map} of URL parameters
     * @throws FormNotFoundException if the form cannot be found
     * @throws FormAlreadySubmittedExceptionif this form has already been submitted and cannot be submitted twice
     * @throws IllegalActivityTypeException if something prevent the form to be skipped
     */
    Map<String, Object> skipForm(final String formID, final Map<String, Object> context) throws FormNotFoundException, FormAlreadySubmittedException, IllegalActivityTypeException;
     ...
     * 
     * @param context Map of context (containing the URL parameters and other data) (including the formID)
     * @return a {@link Map} of URL parameters. This Map should be empty if there are no forms to display
     * @throws Exception 
     */
    FormURLComponents getAnyTodoListForm(final Map<String, Object> context) throws FormNotFoundException;

    /**
     * Get the directory in which the application resources (like process layout, CSS...) are deployed.<br/>
     * This is not required if you use a dedicated web archive (.war) for each forms application. In this case the HTML files will be loaded from the classpath and the CSS and other web resources in the application directory of the web archive.
     * 
     * @param applicationDeploymentDate the application deployment date
     * @param context context Map of context (containing the URL parameters and other data)
     * @return a {@link File} corresponding to the directory containing the application resources
     * @throws IOException if an error occurs while reading the application resources dir
     */
    File getApplicationResourceDir(Date applicationDeploymentDate, Map<String, Object> context) throws IOException;
    
    /**
     * retrieve the right classloader for the context
     * This method is useful if you need to load some classes because some custom java object that are not in the classpath are used in the validators/transient data
     * 
     * @param context Map of context (containing the URL parameters and other data)
     * @return the {@link ClassLoader} for the given context
     */
    ClassLoader getClassloader(final Map<String, Object> context);

    /**
     * Store the transient data context in the HTTP session
     * @param session HTTP session
     * @param storageKey key for the session entry
     * @param transientDataContext transient data context to store
     * @param context Map of context (containing the URL parameters and other data)
     */
    void storeFormTransientDataContext(HttpSession session, String storageKey, Map<String, Object> transientDataContext, Map<String, Object> context);
     ...
     */
    Map<String, Object> retrieveFormTransientDataContext(HttpSession session, String storageKey, Map<String, Object> context);
